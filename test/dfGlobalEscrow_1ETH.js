...
describe("DFGlobalEscrow_Unit_Tests_ETH", function(){
    ...
    it('receiver cannot fund the escrow', async function(){
        await expect(escrow.connect(receiver).fund('abc1234',100000000000000n, {value:100000000000000n})).to.be.revertedWith("Sender must be Escrow's owner or delegator");
    })

    it('agent cannot fund the escrow', async function(){
        await expect(escrow.connect(agent).fund('abc1234',100000000000000n, {value:100000000000000n})).to.be.revertedWith("Sender must be Escrow's owner or delegator");
    })

    it('should fail if a random address try to fund the escrow', async function(){
        await expect(escrow.connect(randomUser).fund('abc1234',100000000000000n, {value:100000000000000n})).to.be.revertedWith("Sender must be Escrow's owner or delegator");
    })
    ...
    it('only sender can fund the escrow with exact amount', async function() {
        ...
        await expect(escrow.connect(sender).fund(
            'abc1234',100000000000000n,
            {value:100000000000000n})
            )
            .to.emit(escrow,'Funded').withArgs('abc1234',sender.address,100000000000000n);
    })
    ...
    it('should fail if funded with higher amount', async function() {
        await expect(escrow.connect(sender).fund(
            ...
            ).to.be.revertedWith("Must fund for exact ETH-amount in Escrow")
    })

    it('should fail if funded with lower amount', async function() {

            await expect(escrow.connect(sender).fund(
                'abc1234',10000000000000n,
    ...
    it('receiver should be able to fully withdraw released fund', async function(){
        ...
        await expect(escrow.connect(receiver).withdraw('abc1234',100000000000000n)).to.emit(
            escrow,
            'Withdrawn'
        ).withArgs('abc1234',receiver.address,100000000000000n);
    })
    ...
    it('receiver can withdraw partly multiple times', async function(){
        ...
        escrow.connect(receiver).withdraw('abc1234',20000000000000n);
        escrow.connect(receiver).withdraw('abc1234',10000000000000n);
        escrow.connect(receiver).withdraw('abc1234',20000000000000n);
        await expect(escrow.connect(receiver).withdraw('abc1234',50000000000000n)).to.emit(
            escrow,
            'Withdrawn'
    ...
    it('should fail if receiver try to withdraw more than funded in smaller amounts multiple time', async function(){
        ...
        escrow.connect(receiver).withdraw('abc1234',20000000000000n);
        escrow.connect(receiver).withdraw('abc1234',10000000000000n);
        escrow.connect(receiver).withdraw('abc1234',30000000000000n);
        await expect(escrow.connect(receiver).withdraw('abc1234',60000000000000n)).to.be.revertedWith("Cannot withdraw more than the deposit");
    })
    ...
    it('sender can withdraw full funds if reverted', async function(){
        ...
        await expect(escrow.connect(sender).withdraw('abc1234',100000000000000n)).to.emit(
            escrow,
            'Withdrawn'
        ).withArgs('abc1234',sender.address,100000000000000n);
    })
})